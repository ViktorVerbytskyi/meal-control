...
export class RegistrationComponent implements OnInit {
  form = this.fb.group({
    ...
    'email': ['', [Validators.required, Validators.email], [this.asyncValidators.checkUserForEmail()]],
  ...
  onSubmit(): void {
    if (this.password.value === this.password2.value) {
      ...
      this.usersService.createNewUser(user).subscribe((user: User) => {
        this.messageService.openSnackBar(MessageText.RegistrationIsSuccessful, MessageType.Success);
        this.router.navigate(['/login']);
      });
    } else {
      this.messageService.openSnackBar(MessageText.PasswordsAreNotTheSame, MessageType.Error);
    }
  }
